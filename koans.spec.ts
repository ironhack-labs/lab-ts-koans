describe("TypeScript Koans - Test Suite 1", () => {
  it("type inference", () => {
    ...
    // Se verifica el tipo por cada opcion por separado
    if (typeof numberOrString === "number") {
      expect(numberOrString).toBe(42);
    } else if (typeof numberOrString === "string") {
      expect(numberOrString).toBe("forty-two");
    }

    // Failing Test 3: Uncomment the expect statement below and provide the expected type
    expect(typeof numberOrString).toBe("number" || "string");
    ...
    expect(typeof nanCheck).toBe("boolean");
  });
});
...
describe("TypeScript Koans - Test Suite 2", () => {
  it("objects in TypeScript", () => {
    ...
    // expect(user.address).toEqual(); // La propiedad 'address' no existe en el User interface
    ...
    expect(Object.keys(student)).toEqual(["name", "age", "gender"]);
    expect(Object.values(student)).toEqual(["Alice", 22, "female"]);
    expect(Object.entries(student)).toEqual([
      ["name", "Alice"],
      ["age", 22],
      ["gender", "female"],
    ]);
...
describe("TypeScript Koans - Test Suite 5", () => {
  it("enums in TypeScript", () => {
    ...
    expect(DaysOfWeek[today]).toEqual("Wednesday");
    ...
    expect(message).not.toEqual("It's Wednesday!");
    expect(message).not.toEqual("It's Sunday!");
    expect(message).not.toEqual("It's Saturday!");
    ...
    expect(upDirection).not.toEqual("DOWN");
  });
});
...
class SportsCar extends Car {
  ...
  private getMakeFromBaseClass(): string {
    ...
    expect(this.make).toEqual("SportsCarMake");
    return this.getMake();
  }
}
...
describe("TypeScript Koans - Test Suite 7", () => {
  it("classes and access modifiers in TypeScript", () => {
    ...
    // expect(sportsCar.getModelFromBaseClass()).toEqual("SportsCarModel");
    // expect(sportsCar.getYearFromBaseClass()).toEqual(2022);
    ...
    // expect(sportsCar.getMakeFromBaseClass()).toEqual("SportsCarMake");
  });
});
