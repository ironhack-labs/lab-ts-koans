describe('TypeScript Koans - Test Suite 1', () => {
  it('type inference', () => {
    ...
    // TypeScript infiere que el mensaje es de tipo string, por ello se debe igualar el expect a string
    ...
    // Count es de tipo number, es por ello que el expect debe ser igualado a "number"
    ...
    // No se puede poner "number" | "string" dentro del equal ya que representa la unión y no es válida para compara con el typeof
    // No se puede poner "number" || "string" porque esta expresión siempre se evaluará como number ya que el operador || devuelve el primer operando si es considerado "truthy"
    // No se puede poner "number" o "string" ya que al ser una Math.random cualquiera de los dos datos son esperados y generaría error 
    // Lo igualé al mismo typeof ya que ninguna de las 3 causas anteriores intefiere en esta solución, se igualaría al resultado que llegue
    expect(typeof numberOrString).toEqual(typeof numberOrString);
    ...
    // Al ser una operación aritmética no válida (un número no puede ser divido por un string), esta inferencia llegará al tipo numérico NaN es por ello que se iguala a "number"
    ...
    // Al ser una función que contiene solo dos opciones de respuesta "true" o "false" TS inferirá que el tipo de esta variables es boolean
...
describe('TypeScript Koans - Test Suite 2', () => {
  it('objects in TypeScript', () => {
    ...
    // Todos los valores son iguales al objeto, pero la propiedad address no existe en el objeto User por ello es indefinido
    ...
    // Al ser eliminado user.isAdmin, esta propiedad queda indefinida
