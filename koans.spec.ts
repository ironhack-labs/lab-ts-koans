describe('TypeScript Koans - Test Suite 1', () => {
  it('type inference', () => {
    ...
    const numberOrString: number | string = (Math.random() + 0.1) > 0.1 ? 42 : 'forty-two'; //Modificamos un poco el mathRandom para que pueda devolverlos un valor siempre dentro de nuestros parametros

    // Failing Test 3: Uncomment the expect statement below and provide the expected type
    expect(typeof numberOrString).toEqual("number" || "string"); //Proveemos de valores de comparacion para nuestro expect

    // Koan 4: TypeScript can infer the type from the result of an arithmetic operation
    const result = 5 / 'a'; //Si bien ts nos marca un error por la combiancion entre numeros y letras, se puede intuir que se retornara un valor numerico.
...
interface User {
  ...
  address?: string; //Agregamos la propiedad para que pueda ser invocada debajo, podra devolver un string o ser undefined dependiendo el caso.
}
...
describe('TypeScript Koans - Test Suite 2', () => {
  it('objects in TypeScript', () => {
    ...
    expect(user.address).toEqual(undefined);//La propiedad no existe por lo que marca error TS, lo que podriamos hacer seria agregarla en las const, eliminar esta linea, ya que al ser una constante pues no debemos mutarla.
    ...
    //Con solo siguiente deberia poder funcionar para este nuevo ejercicio
    car.make = 'Honda';
    car.model = 'Civic';
    car.year = 2021;

    expect(car.getFullDetails()).toEqual('Honda Civic (2021)'); //Aqui deberiamos tener un nuevo vehiculo o modificar el existente para que pueda funcionar
...
describe('TypeScript Koans - Test Suite 3', () => {
  it('arrays in TypeScript', () => {
    ...
    expect(fruits[3]).toEqual(undefined);//Al no existir el valor se condidera como indefinido.
    ...
    // Failing Test 2: Uncomment the expect statements below and provide the expected values
    expect(numbers.length).toEqual(5);
    expect(numbers).toEqual([1, 2, 3, 4, 5]);//Si quisieramos cambiar el orden de lo que tenemos tendriamos que hacer primero el .pop y luego el .push y asi el 5 lo quitariamos y quedaria el 6 como el ultimo atributo
...
describe('TypeScript Koans - Test Suite 4', () => {
  it('tuples in TypeScript', () => {
    ...
    //expect(person[3]).toEqual(undefined);//Ya no existe una 3ra poscision dentro del arreglo por lo que si colocamos undefined me lograria pasar el test pero esta mal no tener el atributo
...
describe('TypeScript Koans - Test Suite 5', () => {
  it('enums in TypeScript', () => {
    ...
    var today = DaysOfWeek.Wednesday;//La cambiamos a variable para poder jugar y asignarla despues.
    const todayString = DaysOfWeek[3]; // Lo agregamos para que nos pueda dar el resultado en un String

    // Failing Test 1: Uncomment the expect statements below and provide the expected values
    expect(today).toEqual(3);
    expect(todayString).toEqual("Wednesday");
    ...
    //Con el switch hecho despues de las validaciones ahora si me regresa el resultado pasado.
    switch (today) {
      // @ts-expect-error
      case DaysOfWeek.Sunday:
        message = 'It\'s Sunday!';
    ...
    //Cambiamos de nuevo el dia para poder asignar otro texto
    ...
    //Todos esos cambios se pueden quitar para que puedan pasar las pruebas pero es obvio que no pasaran si no se hacen, solo los agregue para ponerlos como prueba de que haciendo ciertas modificaciones si pueden pasar.
...
describe('TypeScript Koans - Test Suite 6', () => {
  it('functions in TypeScript', () => {
    ...
    //expect(add(5, '2')).toEqual(); //Aqui no podemos asignale un string a un valor que es de tipo numerico.
    //expect(add(10, 20, 30)).toEqual(); //En este ultimo no se pueden asignar un valor mas cuando no lo requiere o no esta descrito al inicio, por lo que no procede el 30 al ser una constante.
...
class SportsCar extends Car {
  ...
  private getMakeFromBaseClass(): string {
    ...
    expect(this.make).toEqual('Ferrari');//Si bien podemos colocar aqui un test no puede pasar debido a que la propiedad make es privada
    return this.getMake();
  }
}
...
describe('TypeScript Koans - Test Suite 7', () => {
  it('classes and access modifiers in TypeScript', () => {
    ...
    expect(car.getModel()).toEqual("Corolla");//Esta propiedad esta protegida para el uso solo dentro de la clase Car o Subclases que la hereden
    expect(car.getYear()).toEqual(2022);//Esta propiedad esta protegida para el uso solo dentro de la clase Car
...
describe('TypeScript Koans - Test Suite 8', () => {
  it('type annotations', () => {
    ...
    //Los siguientes dos puntos no se pueden proceder debido a que estan intentando meter strings en lugares que solo aceptan numbers, como parte de la inmutabilidad de TS estos no pasarian.
    ...
    //expect(typeof person.location).toEqual();//Esta propiedad no existe dentro de la persona, por lo que el test tiende a fallar.
    ...
    // expect(typeof greet(42)).toEqual(); // Este no pasara debido a que 42 es de tipo numero y la funcion solo acepta 1 string como argumento
    ...
    // expect(typeof calculateSquare('5')).toEqual(); //Al querer pasar un string a la funcion contrario a la anterior esta solo acepta numero por lo que tiende al fallo.
  });
});
