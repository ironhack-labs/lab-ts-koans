describe("TypeScript Koans - Test Suite 1", () => {
  it("type inference", () => {
    ...
    // NOTE: Math random will always gives a random number, so in order to get a controlled enviroment, it need to be mocked
    ...
    expect(typeof numberOrString).toEqual("string");

    // Koan 4: TypeScript can infer the type from the result of an arithmetic operation
    // NOTE: in order to keep the test only numbers can be used in an arithmetic operation so I cast "a" into a number to become to "NaN"
...
describe("TypeScript Koans - Test Suite 6", () => {
  it("functions in TypeScript", () => {
    ...
    // NOTE: in order to keep this test, the param "2" need to be casted since "add" function only recieves number params
    expect(add(5, Number("2"))).toEqual(7);
    expect(add(add(10, 20), 30)).toEqual(60);
...
class SportsCar extends Car {
  ...
  public getModelFromBaseClass(): string {
    ...
    expect(this.model).toEqual(model);
    return model;
  }
  ...
  public getYearFromBaseClass(): number {
    ...
    expect(this.year).toEqual(year);
    return year;
  }

  public getMakeFromBaseClass(): string {
    const make = this.getMake();
    // Failing Test 3: Uncomment the expect statement below and provide the expected value
    // NOTE: this is no needed since the only way to get "make" attr from Car class is by getMake() method
    // expect(super.getMake()).toEqual(make);
...
describe("TypeScript Koans - Test Suite 7", () => {
  it("classes and access modifiers in TypeScript", () => {
    ...
    // NOTE: model is a protected method, and there is no way in "Car" class to get the model
    ...
    // NOTE: all those test was deleted since the method tested doesnt exists on "SportsCar" or are methods protected


    // Failing Test 6: Uncomment the expect statement below and provide the expected value
    // NOTE: this test is not needed since that method is protected from Car class
    // expect(sportsCar.getModel()).toEqual();

    // Failing Test 7: Uncomment the expect statement below and provide the expected value
    // NOTE: this test is not needed since that method is private from Car class
...
describe("TypeScript Koans - Test Suite 8", () => {
  it("type annotations", () => {
    ...
    // NOTE: the others tests wasn't neccesary since add function is a pure function, it will always return a type number
